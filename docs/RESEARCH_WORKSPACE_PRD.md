...
- 通过 arXiv、Crossref、Semantic Scholar 检索候选论文，保存 SearchHit 元数据并支持 DOI/标题去重。
- 检索结果必须经过研究者选择；仅 HTTPS 且响应为 PDF 的结果可由 `paper fetch` 自动导入全文。

### 暂不完成

- 自动判断“最新文献”或声称检索结果穷尽。
- 将 SearchHit 元数据直接当作证据；没有全文时仍必须手动导入。
- 多人权限、云同步和 Web UI。
- 自动替研究者决定最终选题。
...
loi paper search|results|fetch|add|list|analyze
...
在无网络测试中，使用模拟连接器和 LLM 完成：

1. 初始化项目并持久化。
2. 从三个连接器解析统一 SearchHit，并按 DOI/标题去重。
3. 保存、查看检索结果；拒绝把元数据直接送入证据分析。
4. 下载并校验一个模拟 HTTPS PDF，再导入项目。
5. 导入至少两篇带定位符的文本论文。
6. 生成并验证 Paper Cards。
7. 合成带有效证据引用的 Gap。
8. 生成完整 Idea，完成一次 Critique 和一次 Revision。
9. 人工 shortlist 两个明确版本。
10. 以快照形式进入 Arena，并可恢复查看映射关系。
11. 导出同时包含检索线索、证据、推断、版本、批判和 Arena 信息的报告。
